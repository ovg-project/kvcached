...
// hands its reserved VA range to from_blob() and engine kernels must be
...
// so the benchmark confines itself to one platform.
struct Runtime {
  ...
  Runtime() {
    // Not simply the first platform exposing a GPU: the same card is usually
    // enumerated by both Level Zero and OpenCL, and only the Level Zero one
    // implements sycl_ext_oneapi_virtual_mem, which is the whole point of this
    // benchmark. Order between platforms is not guaranteed and
    // ONEAPI_DEVICE_SELECTOR can change it, so select on the capability rather
    // than on position or backend name -- a device without the aspect would
    // reserve fine and then fail inside physical_mem.
    ...
    if (devices.empty()) {
      throw std::runtime_error(
          "No GPU supporting sycl_ext_oneapi_virtual_mem is available. On "
          "Intel GPUs the extension comes from the Level Zero backend; check "
          "ONEAPI_DEVICE_SELECTOR if it excludes level_zero.");
    }
...
void mem_get_info(int dev_idx, size_t *free_bytes, size_t *total_bytes) {
  ...
  if (!dev.has(sycl::aspect::ext_intel_free_memory)) {
    // Matches csrc/xpu_runtime.cpp: the aspect is present without
    // ZES_ENABLE_SYSMAN on the drivers tested here, so this is not advice to
    // set it -- if the aspect is missing, the driver or device genuinely does
    // not expose the query.
    throw std::runtime_error(
        "XPU device does not report free memory "
        "(sycl::aspect::ext_intel_free_memory unavailable); the benchmark "
        "cannot size its pool on this device or driver.");
  }
