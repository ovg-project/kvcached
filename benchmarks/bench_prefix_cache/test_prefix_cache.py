...
def test_prefix_caching(base_url: str, model: str, num_questions: int = 5):
    ...
    if len(latencies) > 1:
        ...
        print(f"  - 'Cache hit for hash' messages (expected for requests 2+)")
        print(f"  - 'Cached block' messages (expected for request 1)")
    else:
        print(f"\n=== Results ===")
...
def main():
    ...
    try:
        ...
        print(f"  bash benchmarks/simple_bench/start_server.sh vllm --enable-prefix-caching")
        return 1
