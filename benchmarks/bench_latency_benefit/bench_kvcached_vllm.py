...
async def get_trace_requests(
    input_requests: list,
    arrivals: "np.ndarray",  # 1-D int array: arrivals[k] = requests in bin k
    dt: float,
):
    """
    Replay a pre-computed arrival trace.
    arrivals[k] is the number of requests to send in time bin [k*dt, (k+1)*dt).
    Requests within each bin are spread uniformly across the bin.
    Yields (request, current_rate) matching the interface of the other generators.
    """
    start_time = time.perf_counter()
    request_index = 0
    n_bins = len(arrivals)
    ...
    for k in range(n_bins):
        ...
        bin_start = k * dt
        current_rate = m / dt  # instantaneous rate for this bin

        for j in range(m):
            if request_index >= len(input_requests):
                break

            # Spread uniformly within the bin
            send_time = bin_start + (j + 0.5) * dt / m
            current_time = time.perf_counter() - start_time
            if send_time > current_time:
                await asyncio.sleep(send_time - current_time)

            yield input_requests[request_index], current_rate
            request_index += 1

    # Drain any remaining requests at zero rate (shouldn't happen normally)
    while request_index < len(input_requests):
        yield input_requests[request_index], 0.0
        request_index += 1
...
async def benchmark(
    *,
    ...
    trace_arrivals: Optional["np.ndarray"] = None,  # 1-D int array from workload trace
    trace_dt: float = 1.0,
):
    ...
    try:
        ...
        if trace_arrivals is not None:
            print(f"Trace mode: replaying {int(trace_arrivals.sum())} arrivals over "
                  f"{len(trace_arrivals) * trace_dt:.0f}s ({len(trace_arrivals)} bins × dt={trace_dt}s)")
            request_generator = get_trace_requests(input_requests, trace_arrivals, trace_dt)
        elif ramp_up_strategy == "ramp-up-down":
...
def main(args: argparse.Namespace):
    ...
    if args.trace is not None:
        trace_data = np.load(args.trace)
        trace_arrivals = trace_data["arrivals"][args.inst - 1].astype(np.int32)
        trace_dt = float(trace_data["dt"])
        total_arrivals = int(trace_arrivals.sum())
        print(f"Loaded trace: inst={args.inst}, total arrivals={total_arrivals}, "
              f"dt={trace_dt}s, bins={len(trace_arrivals)}")
        # Override num_prompts to match the trace
        args.num_prompts = total_arrivals
        print(f"Setting --num-prompts={args.num_prompts} from trace.")
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to workload_trace.npz generated by plot_workload_design.py. "
        "When set, --inst selects which instance's arrival schedule to replay. "
        "Overrides ramp-up-down and request-rate settings.",
    )
    parser.add_argument(
        "--inst",
        type=int,
        default=1,
        help="1-indexed instance number to select from the trace (default: 1). "
        "Maps to arrivals[inst-1] in the .npz file.",
    )
