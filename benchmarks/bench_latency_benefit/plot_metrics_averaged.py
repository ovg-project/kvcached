            ...
            for subplot_idx, (ax, metric) in enumerate(zip([ax1, ax2], metric_pair)):
                ...
                for reqrate in reqrates:
                    ...
                    for key, metrics_data in data['true'].items():
                        if key[0] == reqrate and key[1] == comp_len:  # reqrate and completion_len match
                ...
                # Plot bars
                ax.bar(x_positions - bar_width/2, true_values,
                       bar_width, label='KV Cache',
                       color='#1f77b4', alpha=0.8)
                ax.bar(x_positions + bar_width/2, false_values,
                       bar_width, label='No Cache',
                ...
                for i, (true_val, false_val) in enumerate(zip(true_values, false_values)):
                    if true_val > 0 and false_val > 0:
                        ...
                        ax.text(x_positions[i], y_pos, f'{gain:.1f}x',
                ...
                ax.set_title(f'{metric_type} {metric_display_name} - Completion Length {comp_len} (Averaged across models)')
