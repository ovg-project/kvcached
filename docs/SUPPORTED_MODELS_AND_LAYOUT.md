...
滑窗层与全注意力层 **`block_size`/`cell_size` 不同但 `block_mem_size` 相同** 的模型(如 **Gemma 3/4**:滑窗 `block_size=64`/`cell=1024`、全注意力 `block_size=16`/`cell=4096`,两者 `block_mem_size=65536` 相同)**现已支持**。
