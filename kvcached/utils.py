...
def normalize_gpu_device(device: str) -> str:
    ...
    PyTorch-ROCm and the C++ extension (``torch::stable::Device``) address AMD
    GPUs as ``cuda``; kvcached's integration accepts ``hip`` strings, so
    normalize them before handing the device to ``get_device_module()`` or
    ``create_kv_tensors``.

    ``xpu[:N]`` is returned unchanged: Intel GPUs are a distinct PyTorch device
    type (``DeviceType::XPU``), addressed by ``torch.xpu`` and parsed as-is by
    ``torch::stable::Device``. Rewriting it to ``cuda`` would misroute the
    allocation.
    """
    dev = str(device)
