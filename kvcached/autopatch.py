...
def _patch_vllm(_vllm: types.ModuleType) -> None:
    ...
    # Log overall status
    successful_patches = [
        name for name, success in patch_status.items() if success
    ]
    failed_patches = [
        name for name, success in patch_status.items() if not success
    ]
...
def _patch_kv_cache_coordinator(kvcoord_mod: types.ModuleType,
    ...
    # Idempotent: avoid double-wrapping
...
def _patch_gpu_model_runner(gpumr_mod: types.ModuleType) -> bool:
    ...
    if not hasattr(GPUModelRunner, "_allocate_kv_cache_from_kvcached"):
        ...
        def _allocate_kv_cache_from_kvcached(self, kv_cache_config):
            import torch  # local import to avoid import-time costs
    ...
    candidate_method = _find_kv_cache_init_method(GPUModelRunner)
    if candidate_method is not None:
        original_method = getattr(GPUModelRunner, candidate_method)

        def _patched_allocator(self, kv_cache_config, *args: Any,
                               **kwargs: Any):
        ...
        if getattr(original_method, "__kvcached_patched__", False) is not True:
            _patched_allocator.__kvcached_patched__ = True  # type: ignore[attr-defined]
            setattr(GPUModelRunner, candidate_method, _patched_allocator)

    return True


def _find_kv_cache_init_method(cls: type) -> Optional[str]:
    # Heuristic: find first method whose source mentions _allocate_kv_cache_tensors
    for name, fn in inspect.getmembers(cls, predicate=inspect.isfunction):
        try:
            src = inspect.getsource(fn)
        except Exception:
            continue
        if "_allocate_kv_cache_tensors(" in src and "_reshape_kv_cache_tensors(" in src:
            return name
    # Fallbacks: common method names in different versions
    for name in (
            "allocate_kv_cache",
            "_allocate_kv_cache",
            "initialize_kv_cache",
