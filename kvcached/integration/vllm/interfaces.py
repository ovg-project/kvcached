...
def build_kv_views(
    ...
    if not is_mla and len(kvcache_shape) == 4:
        ...
        if padded_page_size_bytes not in (None, logical_page_size_bytes):
            raise NotImplementedError("kvcached: padded packed KV pages are not supported")
        ...
        return views, logical_page_size_bytes
    ...
    if not _contiguous_layout:
        ...
        if is_mla:
            if page_size_bytes == logical_page_size_bytes:
                ...
                strides = list(torch.empty(kernel_kvcache_shape, device="meta").stride())
