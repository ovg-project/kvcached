...
class SGLangPrometheusMetricsExporter:
    ...
    def _set_pool_gauges(self, snapshots: List[Dict[str, Any]]) -> None:
        ...
        for snapshot in snapshots:
            ...
            for field, gauge in self._pool_gauges.items():
                value = snapshot.get(field)
                # An absent target is not the previous target or a zero-byte limit.
                gauge.labels(**labels).set(float(value) if value is not None else float("nan"))
    ...
    def record_update_error(self) -> None:
        # The metric backend may be the reason update() failed in the first place.
        try:
    ...
    class KVCachedSchedulerMetricsCollector(base_cls):  # type: ignore[misc, valid-type]
        ...
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            ...
            try:
                ...
                # SGLang constructs collectors on all ranks but calls log_stats
                # only on reporting ranks. Do not publish stale startup samples.
