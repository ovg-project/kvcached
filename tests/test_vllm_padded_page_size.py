...
def interfaces(monkeypatch):
    ...
    def _cpu_create(ftensor_bytes_per_layer, _itemsize, _device, num_layers,
        ...
        if mod._contiguous_layout:
            return [
                torch.empty(ftensor_bytes_per_layer * num_layers, dtype=torch.uint8)
            ]
        return [
...
@pytest.mark.parametrize("attention_type", ["MLA", "MHA"])
@pytest.mark.parametrize("contiguous", [False, True])
@pytest.mark.parametrize("entry", ["alloc", "views"])
def test_padded_stride_calculation_does_not_allocate_storage(
    interfaces, monkeypatch, attention_type, contiguous, entry
):
    monkeypatch.setattr(interfaces, "_contiguous_layout", contiguous)
    original_empty = torch.empty
    metadata_shapes = []

    def checked_empty(size, *args, **kwargs):
        if isinstance(size, (tuple, list)):
            assert kwargs.get("device") == "meta"
            metadata_shapes.append(tuple(size))
        return original_empty(size, *args, **kwargs)

    monkeypatch.setattr(torch, "empty", checked_empty)
    shape = (4, BLOCK_SIZE, 8) if attention_type == "MLA" else FLASH_INFER_SHAPE
    if entry == "alloc":
        interfaces.alloc_kv_cache(
            shape, BLOCK_SIZE, DTYPE, "cuda:0", NUM_LAYERS,
            attention_type=attention_type,
            padded_page_size_bytes=PADDED_PAGE_BYTES,
        )
    else:
        interfaces.build_kv_views(
            _raw_pools(), shape, BLOCK_SIZE, DTYPE, attention_type, 4,
            4 * PADDED_PAGE_BYTES, NUM_LAYERS,
            padded_page_size_bytes=PADDED_PAGE_BYTES,
        )
    # Split K/V strides are calculated arithmetically; the other paths use meta.
    if attention_type == "MLA" or contiguous:
        assert len(metadata_shapes) == 1
...
@pytest.mark.parametrize("padded", [None, LOGICAL_PAGE_BYTES])
@pytest.mark.parametrize("layout", ["NHD", "HND"])
def test_packed_unpadded_allocation_preserves_main_geometry(interfaces, monkeypatch, padded, layout):
    monkeypatch.setattr(interfaces, "_contiguous_layout", False)
    shape = (4, 1, BLOCK_SIZE, 8)
    views, meta = interfaces.alloc_kv_cache(
        shape, BLOCK_SIZE, DTYPE, "cuda:0", NUM_LAYERS,
        attention_type="MHA", kv_layout=layout, return_meta=True,
        padded_page_size_bytes=padded,
    )
    assert meta["page_size_bytes"] == LOGICAL_PAGE_BYTES
    expected_blocks = (8 * PAGE_SIZE) // NUM_LAYERS // LOGICAL_PAGE_BYTES
    assert meta["num_blocks_per_layer"] == expected_blocks
    assert tuple(views[0].shape) == (expected_blocks, *shape[1:])


@pytest.mark.parametrize("entry", ["alloc", "views"])
def test_padded_packed_pages_keep_explicit_unsupported_boundary(interfaces, entry):
    shape = (4, 1, BLOCK_SIZE, 8)
    with pytest.raises(NotImplementedError, match="padded packed KV pages"):
        if entry == "alloc":
            interfaces.alloc_kv_cache(
                shape, BLOCK_SIZE, DTYPE, "cuda:0", NUM_LAYERS,
                padded_page_size_bytes=PADDED_PAGE_BYTES,
            ...
            interfaces.build_kv_views(
                _raw_pools(), shape, BLOCK_SIZE, DTYPE, "MHA", 4,
                4 * PADDED_PAGE_BYTES, NUM_LAYERS,
                padded_page_size_bytes=PADDED_PAGE_BYTES,
            )
