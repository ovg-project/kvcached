...
def test_release_during_growth_is_not_absorbed_into_rejected_epoch(monkeypatch):
    manager = _manager()
    now = [10.0]
    epoch = [(1, 100)]
    manager._physical_growth_capacity_epoch_provider = lambda: epoch[0]
    monkeypatch.setattr("kvcached.kv_cache_manager.time.monotonic", lambda: now[0])
    attempted_epoch = manager._read_physical_growth_capacity_epoch()
    epoch[0] = (2, 200)  # A peer releases while prepare/reconciliation runs.
    manager._record_physical_growth_result(
        {"physical_growth_capacity_rejections_total": 1}, capacity_epoch=attempted_epoch)
    manager._physical_growth_retry_after = 15.0
    now[0] += 0.011
    assert not manager._physical_growth_retry_is_blocked()
    assert manager._get_operation_counter("physical_growth_capacity_wakeups_total") == 1
    assert manager._get_operation_counter("physical_growth_retry_probes_total") == 0
