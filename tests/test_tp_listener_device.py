...
def test_listener_thread_restores_its_own_device(monkeypatch, tmp_path, backend):
    ...
    class FakeThread:
        ...
        # Nothing was started, so the listener drains immediately. #480 made
        # this reachable: starting a listener now stops whichever one is
        # registered for that rank, and a stop that cannot join fails the start.
    ...
    # A throwaway registry, restored on teardown: the listener built here holds
    # a fake socket and a fake thread, and leaving it in the real one would hand
    # those to the next start_worker_listener_thread() or to the atexit stop.
    monkeypatch.setattr(tp_ipc_util, "_listeners", {})
