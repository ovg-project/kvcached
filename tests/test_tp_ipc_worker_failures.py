...
@pytest.mark.parametrize(
    ("command", "expected_status", "combined"),
    [
        ("prepare_unmap_from_kv_tensors", "prepared", False),
        ("prepare_unmap_from_kv_tensors", "committed", True),
        ("commit_unmap_from_kv_tensors", "committed", False),
        ("abort_unmap_from_kv_tensors", "aborted", False),
    ],
)
def test_worker_executes_unmap_transaction_phases(monkeypatch, command, expected_status, combined):
    ...
    assert calls
    if combined:
        assert calls == [("prepare", [7], "txn-1", 3), ("commit", "txn-1", 3)]
