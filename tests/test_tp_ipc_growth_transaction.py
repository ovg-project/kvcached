...
def _restore_tp_ipc_util(previous):
    module = sys.modules.pop("kvcached.tp_ipc_util", None)
    if module is not None:
        # This module only creates inert listener/thread fakes.
        module._listeners.clear()
...
def test_fatal_native_state_never_becomes_recoverable_or_committed(monkeypatch, phase):
    ...
    async def exchange(rank, message, pp_rank=0):
        ...
        if rank == 1 and command == phase:
            # A stale registry state must not overrule the native safety error.
            return {
                "status": "error", "error_type": "state_consistency",
                "transaction_state": "reserved", "message": "cleanup failed",
            }
        return _success_response(command)

    monkeypatch.setattr(module, "_send_and_receive_message", exchange)
    try:
        with pytest.raises(module.MapTransactionOutcomeUnknownError, match="cleanup failed"):
            module.broadcast_map_to_kv_tensors(2, [0])
        assert "finalize_map_to_kv_tensors" not in events
        assert "abort_prepared_map" not in events
        assert "orphan_map_transaction" not in events
...
def test_unreconciled_outcome_is_terminal_without_cleanup_or_retry(monkeypatch, phase):
    ...
    try:
        ...
        assert issubclass(module.MapTransactionOutcomeUnknownError, module.StateConsistencyError)
    finally:
        _restore_tp_ipc_util(previous)
