    ...
    def add_request(self, request_stats: RequestStats):
        ...
        if request_stats.success:
            ...
            if request_stats.response_time:
                ...
                total_time = self.avg_response_time * (self.successful_requests - 1)
                self.avg_response_time = (total_time + request_stats.response_time) / self.successful_requests
        else:
...
class TrafficMonitor:
    ...
    def __init__(self, idle_threshold_seconds: int = 300):
        self.model_stats: Dict[str, ModelActivityStats] = defaultdict(ModelActivityStats)
    ...
    def record_request_start(self, model_name: str, endpoint_path: str) -> RequestStats:
    ...
    def record_request_end(self, request_stats: RequestStats, success: bool = True, 
        ...
        with self._lock:
            self.model_stats[request_stats.model_name].add_request(request_stats)
    ...
    def get_idle_models(self, idle_threshold_seconds: Optional[int] = None) -> List[str]:
    ...
    def get_active_models(self, idle_threshold_seconds: Optional[int] = None) -> List[str]:
