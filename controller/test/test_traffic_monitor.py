...
class TrafficMonitorTest:
    ...
    async def test_endpoint(self, session: aiohttp.ClientSession, endpoint: str, method: str = "GET", json_data: Dict = None) -> Dict[str, Any]:
    ...
    async def generate_test_traffic(self, session: aiohttp.ClientSession, num_requests: int = 5) -> List[Dict]:
        ...
        for i in range(num_requests):
            ...
            result = await self.test_endpoint(session, "/v1/completions", "POST", test_request)
            results.append({
                "request_id": i+1,
                "model": model,
                "prompt": prompt,
            ...
            if result["success"]:
                ...
                if isinstance(result["data"], dict):
                    ...
                    if "traffic_stats" in data:
                        ...
                        print(f"      Rate: {model_stats.get('request_rate', 0):.2f} req/s")
            else:
    ...
    async def test_sleep_endpoints(self, session: aiohttp.ClientSession):
        ...
        if result["success"]:
            ...
            data = result["data"]
            print(f"  💤 Sleeping models: {len(data.get('sleeping_models', {}))}")
            print(f"  🎯 Sleep candidates: {len(data.get('sleep_candidates', []))}")
        ...
        # Put model to sleep
        result = await self.test_endpoint(session, f"/sleep/model/{encoded_test_model}", "POST")
        ...
        if result["success"]:
            ...
            if test_model in sleeping or any(test_model in str(k) for k in sleeping.keys()):
        ...
        result = await self.test_endpoint(session, f"/wake/model/{encoded_test_model}", "POST")
    ...
    async def test_integration_scenarios(self, session: aiohttp.ClientSession):
        ...
        successful_requests = sum(1 for r in traffic_results if r["result"]["success"])
        print(f"  Generated traffic: {successful_requests}/{len(traffic_results)} successful requests")
        ...
        if stats_result["success"]:
            ...
            for model_name, model_stats in stats.items():
                if model_stats.get("total_requests", 0) > 0:
                    print(f"    - {model_name}: {model_stats['total_requests']} requests, "
        ...
        idle_result = await self.test_endpoint(session, "/traffic/idle?threshold=5")
        ...
        # Put model to sleep
        await self.test_endpoint(session, f"/sleep/model/{encoded_test_model}", "POST")
        await asyncio.sleep(2)
        ...
        request_result = await self.test_endpoint(session, "/v1/completions", "POST", test_request)
        
        if request_result["success"]:
            print("  ✅ Request succeeded - wake-on-request working")
        else:
            print(f"  ❌ Request failed: {request_result.get('error', 'Unknown error')}")
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                if not health_result["success"]:
                    print(f"❌ Server not responding: {health_result.get('error', 'Unknown error')}")
                    return
                ...
                if models_result["success"]:
                    ...
                    self.test_models = [m for m in self.test_models if m in available_models]
                    if not self.test_models:
                        print("⚠️  No test models available, using first available model")
                ...
                traffic_test_results = await self.test_traffic_endpoints(session)
                ...
                successful_traffic_tests = sum(1 for r in traffic_test_results.values() if r["success"])
                
                total_sleep_tests = len(sleep_test_results) 
                successful_sleep_tests = sum(1 for r in sleep_test_results.values() if r["success"])
                
                print(f"Traffic Tests: {successful_traffic_tests}/{total_traffic_tests} passed")
                print(f"Sleep Tests: {successful_sleep_tests}/{total_sleep_tests} passed")
...
async def main():
    ...
    parser.add_argument('--url', default='http://localhost:8081', 
                        help='Base URL for the controller server (default: http://localhost:8081)')
