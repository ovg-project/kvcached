...
def run_benchmark_client(model_name, port, output_queue, stop_event):
    ...
    output_queue.append(f"Starting benchmark for {model_name} with command: {' '.join(command)}")
    ...
    try:
        ...
        output_queue.append(f"Process started for {model_name}, PID: {process.pid}")
        ...
        output_queue.append(f"=== Benchmark completed for {model_name} with return code {return_code} ===")
        
    except Exception as e:
        output_queue.append(f"Error running benchmark for {model_name}: {str(e)}")
        import traceback
...
def main_display(stdscr, models_config, port):
    ...
    try:
        ...
        if len(model_names) != 2:
            stdscr.addstr(0, 0, f"Error: Expected 2 models, found {len(model_names)}")
            stdscr.refresh()
            stdscr.getch()
            return
        ...
        thread1 = threading.Thread(
            target=run_benchmark_client,
            args=(model1_name, port, output_queues[0], stop_events[0])
        )
        thread2 = threading.Thread(
            target=run_benchmark_client,
            args=(model2_name, port, output_queues[1], stop_events[1])
        )
        ...
        try:
            while True:
                ...
                win1.addstr(0, 0, header1[:mid_col-2], curses.color_pair(2) | curses.A_BOLD)
                win2.addstr(0, 0, header2[:width-mid_col-1], curses.color_pair(3) | curses.A_BOLD)
                ...
                for i, (window, queue) in enumerate(zip(windows, output_queues)):
                    lines = list(queue)
                    ...
                    for j, line in enumerate(lines[start_idx:end_idx]):
                        if j + 1 < max_lines:
                            ...
                            max_width = (mid_col - 2) if i == 0 else (width - mid_col - 1)
                ...
                status_win.addstr(0, 0, status[:width-1], curses.color_pair(1))
                ...
                if key == ord('q') or key == ord('Q'):
                    ...
                    scroll_pos[0] = min(len(output_queues[0]) - max_lines, scroll_pos[0] + 1)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Run benchmark clients for two models')
    parser.add_argument('port', type=int, help='Server port to connect to')
    parser.add_argument('--config', default='two_models_config.json', 
    ...
    if len(models_config) != 2:
        print(f"Error: Configuration must contain exactly 2 models, found {len(models_config)}")
        sys.exit(1)
    
    model_names = list(models_config.keys())
    print(f"Starting benchmark clients for models: {model_names[0]} and {model_names[1]}")
    ...
    if args.no_curses:
        ...
        thread1 = threading.Thread(
            target=run_benchmark_client,
            args=(model_names[0], args.port, output_queues[0], stop_events[0])
        )
        thread2 = threading.Thread(
            target=run_benchmark_client,
            args=(model_names[1], args.port, output_queues[1], stop_events[1])
        )
