...
- **[2026-04]** Added **prefix caching** support. kvcached now supports **automatic prefix caching (APC)** for vLLM and **RadixCache** for SGLang. Evictable blocks are retained for cross-request prefix reuse and evicted on demand when memory is needed. The cached token budget can be controlled via `KVCACHED_MAX_CACHED_TOKENS` (default: `16000`) for both engines.
