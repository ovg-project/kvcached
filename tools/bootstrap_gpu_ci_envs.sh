...
create_env core "${CORE_TORCH_SPEC}"
create_env vllm "${VLLM_SPEC}"
create_env sglang "${SGLANG_SPEC}"

cat >"${ENV_ROOT}/github-actions.env" <<EOF
KVCACHED_GPU_PYTHON=${ENV_ROOT}/core/bin/python
KVCACHED_VLLM_PYTHON=${ENV_ROOT}/vllm/bin/python
KVCACHED_SGLANG_PYTHON=${ENV_ROOT}/sglang/bin/python
EOF
