...
from engine_compat_artifact import allowed_paths, load_envelope, materialize, pack
from repair_engine_compat import run_command, tail

ROOT = Path(__file__).resolve().parents[1]
POLICY = ROOT / ".github/engine-compat/vllm-allow.json"
...
def checks() -> int:
    ...
    tests = ["tests/test_vllm_virtual_kv_capacity.py"]
    # Probes use the immutable controller tests; candidate tests run separately.
    if os.environ.get("ENGINE_COMPAT_PROBES") == "1":
        tests = [str(Path.cwd() / Path(name).name) for name in tests]
...
def cpu(args) -> None:
    ...
    checks_root.mkdir()
    for name in ("test_vllm_virtual_kv_capacity.py",):
        shutil.copy2(ROOT / "tests" / name, checks_root / name)
    command = [sys.executable, str(Path(__file__).resolve()), "checks"]
    write_json(
...
def gpu(args) -> None:
    ...
    head = materialize(args.source, payload, args.base, args.tag, allowed_paths(POLICY))
    write_json(args.output / "candidate.json", payload)
    container_name = "kvcached-compat-" + uuid.uuid4().hex
    env = dict(os.environ, COMPAT_CONTAINER_NAME=container_name)
    command = [
        "bash",
...
def main() -> int:
    ...
    if args.stage == "checks":
        if args.probes:
            os.environ["ENGINE_COMPAT_PROBES"] = "1"
        return checks()
